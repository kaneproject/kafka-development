...
public class ConsumidorClima {
	...
	public static void main(String[] args) throws IOException {
		...
		KafkaConsumer<String, Clima> kafkaConsumer = new KafkaConsumer<>(properties);
		Thread thread = new Thread(kafkaConsumer::close);
		Runtime.getRuntime().addShutdownHook(thread);
		
		kafkaConsumer.subscribe(Collections.singletonList(Stream03Topology.TOPIC_DESTINO));
		while(true) {
			ConsumerRecords<String, Clima> records = kafkaConsumer.poll(Duration.ofMillis(100));
